...
- **送达判定按平台看回执，不只看 HTTP 码**——钉钉、飞书会在 HTTP 200 的响应体里报错（如钉钉 310000「keywords not in content」）。出现这种情况时日志会如实说「通知被 <平台> 拒收」并带出平台给出的原因，响应体不是预期结构时说「无法判定」；自建 webhook 没有可依据的回执契约，HTTP 200 即算成功（会注明送达状态无法判定）
...
1. 编辑仓库根目录的 `images.lock.txt`，写入需要的镜像（每行一个，`#` 开头为注释；末尾的换行不是必须的）
2. 触发 `Sync-Batch` 工作流
...
同时，**同步模式下**`--dry-run` 没有真的搬过任何东西，因此**报告、Step Summary 与通知里的结果字段都为空或零**：报告里的耗时恒为 0（耗时排行也随之隐去）、`dest_digest` 留空（`source_digest` 照常记录，它描述的是源镜像本身）。`--write-lock` 与 `--notify-webhook` 因为没有对象而不生效——前者记录的是「这次推上去的是哪一份」，后者通报的是「这次搬得怎么样」——显式传入时会有一条告警说明。要生成锁文件或发送通知，去掉 `--dry-run` 再跑一次。

（终端那行「总耗时」是例外：它描述的是**进程**跑了多久，与「这次搬了多少」无关，因此照常显示、也是真实的。）
