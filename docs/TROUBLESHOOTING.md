...
| `通知被 ... 拒收` / `无法判定` / `通知发送失败` | [通知没送到](#通知被平台拒收或无法判定) |
...
同步本身没问题（退出码与镜像结果都不受影响），但**群里没收到通知**。日志里会出现下面
三种告警之一（原文照抄，`<平台>` 是 `dingtalk` / `feishu` / `slack`）：

```text
[警告] 通知被 dingtalk 拒收（HTTP 200，errcode=310000 errmsg=keywords not in content），结果不受影响
[警告] 通知发送结果无法判定（HTTP 200，响应体不是 JSON），结果不受影响
[警告] 通知发送失败（HTTP 502，bad gateway），结果不受影响
```

### 原因

**HTTP 码只说明「对方应答了」。** 钉钉与飞书恰好在 **HTTP 200 的响应体里**用 `errcode` /
`code` 报错——只看码的话，平台侧拒收会被报成「结果已推送到」这个假的成功。判定因此改为
按平台解析响应体（行为说明见 [USAGE.md](USAGE.md) 的「配置同步结果通知」一节），三句话
互不混淆：

| 告警 | 含义 |
| --- | --- |
| `通知被 <平台> 拒收` | 平台**明确拒绝**了这次推送，原因就在它给出的 `errmsg` / 报错文本里 |
| `通知发送结果无法判定` | 响应体不是该平台的预期结构（空体 / 不是 JSON / 没有对应字段）。判不出来就说判不出来，**不默认成功** |
| `通知发送失败（HTTP <码>）` | HTTP 码本身非 2xx，问题在代理 / 网关这一层，与平台配置无关 |

`generic`（自建 webhook）没有可依据的回执契约，HTTP 2xx 仍算成功，日志会注明「该类型无
统一回执，送达状态无法判定」。

### 解决

1. **先读日志里平台给出的原因**——`errmsg` / `code` / 报错文本是平台自己写的，它才是
   拒收的真实原因，比任何猜测都准
2. 拿这个原因去核对机器人一侧的配置：安全设置（关键词、签名、IP 白名单）、token 是否
   失效、是否被限流
3. 判「无法判定」时，**先看链路上有没有中间层**：透明代理 / 网关会把回执改写掉（200 回来
   一页 HTML 是很典型的一种）。平台自己的回执不长这样

### 为什么通知失败不影响同步结果

通知是**附加能力**：发不出去只告警，`sync.sh` 的退出码与镜像结果都不受影响（
`notify_send_text` 永远返回 0）。这一条是刻意的设计——通知挂了不该让一次成功的同步变红。
