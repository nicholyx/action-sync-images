...
- **通知的送达判定改为按平台解析响应体**（[#159](https://github.com/nicholyx/action-sync-images/issues/159)）。此前判定只看 HTTP 码（`curl -o /dev/null -w '%{http_code}'` 再判 `^2`），而钉钉、飞书恰好在 **HTTP 200 的响应体里**用 `errcode` / `code` 报错——于是平台侧拒收（如钉钉 310000「keywords not in content」、飞书 11232 限流）被报成「结果已推送到 <平台>」。一个假的成功通知比不发通知更坏：收件人不再去看日志。这与项目记过的「恒真的断言比没有断言更糟」同族——那条判定对**任何** HTTP 200 都成立，包括「对方明确拒绝了」
  - 判定**按平台分别实现**（响应体的字段名、成功值、形态三者本就不同，不造统一抽象）：钉钉看 `errcode`、飞书看 `code`（`StatusCode` / `StatusMessage` 是官方标注的冗余字段，只在 `code` 缺失时回退）、Slack 看纯文本 `ok`（不是 JSON）。拒收时如实报「通知被 <平台> 拒收」并带出平台给出的原因；响应体不是预期结构时报「无法判定」——**已送达 / 被拒收 / 无法判定三者互不混淆**，判不出来绝不默认成功
  - **Slack 只把「像回执 token 的响应体」算拒收**：透明代理返回的 200 HTML 错误页走的也是这条路径，报成「平台拒收」会把排查方向带偏到机器人配置上——它不是 Slack 的回执，归「无法判定」
  - **自建 webhook（`generic`）不猜成败**：它没有可依据的回执契约，HTTP 2xx 仍算成功，并如实注明「该类型无统一回执，送达状态无法判定」——这是刻意的边界，不是遗漏
  - `notify_send_text` 的既有契约不变：**永远返回 0**，通知发不出去只告警，运行结果与退出码不受影响；日志仍绝不输出 webhook URL。**响应体落盘引入了旧实现没有的失败面**：`mktemp` 建不出临时文件（`TMPDIR` 不可写这类环境）时退成 `/dev/null`，通知照发、判定如实报「无法判定」——不让它冒出去（`set -e` 会穿透函数，一次成功的同步会因为通知这个附属动作变红）
  - **非 2xx 的既有文案一字未改**：空响应体时仍是 `通知发送失败（HTTP 502），结果不受影响`，带响应体时在其后追加首行——日志检索习惯不失效
  - **改前行为先复现**：本地假 webhook 返回 `200 + {"errcode":310000,"errmsg":"keywords not in content"}`，改前输出「结果已推送到 dingtalk」，改后输出「通知被 dingtalk 拒收（HTTP 200，errcode=310000 errmsg=keywords not in content）」
  - **断言分两层**：抽取式单测喂该平台回执矩阵（含 `errcode:0` → `confirmed` 的正对照），端到端再真发一次（假 webhook 的响应体就是判据）。**变异验证**：把响应体落盘改回 `-o /dev/null` 时**单测全绿而端到端红**——这正是端到端那一步存在的理由；「去掉 `errcode == 0` 判定」「默认结论改成 `confirmed`」「详情限长改大」「只写 stdout 不写全局变量」四个单点变异各自被对应断言抓住；把「夹具永远写空文件」与实现变异叠加，断言仍红（两个错不会相互抵消）。两个新步骤在 **bash 3.2（本机）与 bash 5.3（CI 的解释器）** 下都跑过一遍。顺带修好了 AC5 那条断言本身：它此前把调用写成 `notify_send_text ... || fail`，**`||` 列表会让 errexit 对函数内部失效**，于是它只看得到「返回值是 0」一个形态，内部命令失败（如 `mktemp`）照样假绿——改成与真实调用点一致的裸调用后，两种形态都拦得住
...
- **CI 新增 `lint.sh 自测` job**（[#148](https://github.com/nicholyx/action-sync-images/issues/148)）。`scripts/lint.sh` 此前是 CI 里**被调用为零**的脚本——它自己的职责恰恰是「本地过 = CI 过」，而这条保证的上游一直没有保障：近两轮修的 #138（非 git 目录下根本跑不起来）与 #133（比 CI 宽松）**都出在它身上**。新 job 的两条断言方向相反，正对应那两个缺口：非 git 目录下 actionlint 项不失败、引入真实告警时会红。
  - **不并入 `smoke-test`**：`lint.sh` 的 zizmor 那一项要 docker，而 `smoke-test` 的设计前提是**不启容器**（33 个步骤都不启）——那是那个 job 的性质，不该为一组新断言破坏它
  - 断言只断 **actionlint 那一项**、不断 `lint.sh` 的整体退出码：新 job 里 yamllint 未装、zizmor 要拉镜像，整体 rc 会随环境波动——拿它当判据会变成「对正确实现假报错」
  - `ci-summary` 的 `needs` 因此从 7 项变 8 项；**分支保护勾的是 `ci-summary`，所以不必改仓库设置**（这正是当初只勾一个汇总检查的用意）

- **`./scripts/lint.sh` 在结尾列出「不是 CI 同源」的项**（[#139](https://github.com/nicholyx/action-sync-images/issues/139)）。zizmor 那一项在本机没有 docker 时会退回本机二进制——它**跑了、也算通过**，但那个「通过」与 CI（用 `ci.yml` 里 pin 的镜像）的通过可信度不同。此前这层区别只体现在 zizmor 那一段的两行提示里，**是否被读到取决于读者有没有翻到那里**；现在汇总行下方会单独列出，并写明「其『通过』不等于 CI 的通过」。
  - **不改三个计数**：把「非 CI 同源」升为独立一类是结构改动，与要解决的问题不成比例。凑一段事实性说明已经够（这一点在 #139 里有过专门讨论，另留一条改进方向）
  - 版本偏斜是**双向静默**的：旧版缺某个 audit 会漏报（假绿），不认 `rules:` schema 或豁免集解读不同则让豁免多寡变化（假红或过度豁免）。本机两边恰好同版本是巧合，不是设计保证
