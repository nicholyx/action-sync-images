...
- [ ] 每条 CI 断言先在本地复现（包括 CI 脚本的提取执行）。
      **测「夹具坏」时要改它实际读的那份文件**：夹具若在步骤内部硬编码了固定路径
      （`PATH="/tmp/stub-skopeo:$PATH"`），从外层 `PATH=<别的 stub>:$PATH bash step.sh`
      **完全无效**——步骤用的是它自己那份。2026-09-24 真实踩到：这样测「夹具坏」得到一次
      **假绿**（仍是原夹具在跑），差点据此写下「夹具没问题」的结论（#166）。改固定路径上的
      文件、或先重建夹具，才是有效的测法
...
- [ ] **断言要覆盖「全部」，不是「最后一个」**。`jq -e '.images[] | has("note")'` 在 `-e` 下
      ...
      `jq -e` 还有第二处反直觉：它在 `null` / `false` 这类**合法** JSON 上返回非零，
      拿它当「能不能解析」的探针会把合法响应说成「不是 JSON」——结论没变，**原因与事实不符**。
      判解析成败用 `jq .`，判「是不是对象」用 `jq -e 'type == "object"'`（#159）
- [ ] **断言不得对正确实现假报错**。这是「恒假断言」的镜像问题：一个会把正确实现判红的断言，
      ...
      **比 JSON 对象时要用 `jq -S`**：JSON 的键序**不是语义**，把 summary 的键重排是行为等价的
      改动、报告内容一字不差，而 `jq -c '.summary'` 原样保留文件里的键序，拿它跟写死的字面量
      比就会把正确实现判红（#158 的检查阶段实测）。要么 `jq -S` 排序后比，要么比
      `keys|sort` 与逐字段取值——**判据里不该出现「键在第几位」这种信息**
- [ ] **改动一处「本来不可能失败」的实现时，先问它新增了什么失败面**。通知的发送原本写
      `/dev/null`（不可能失败），改成先落盘临时文件就多出 `mktemp` 建不出文件这条路径——
      而 `set -e` 会穿透函数，于是**一次成功的同步会因为通知这个附属动作以非零退出**，
      「通知是附加能力」的既有契约当场断掉。**新增失败面的兜底要与它替换掉的那条路径等价**：
      退成 `/dev/null` 后通知照发、判定如实报「无法判定」，不伪造成功（#159）
- [ ] **逼出「工具建不出临时文件」这类失败要用函数覆盖，不要用环境变量**。
      `TMPDIR=/nonexistent mktemp` 在 macOS 上**照样成功**（BSD 的 mktemp 不读 TMPDIR），
      这样写出来的用例会在本地**假绿**、只在 Linux 上才红——而「本地假绿」正是本仓库
      最不愿见到的那类绿灯。可靠写法是在调用前 `mktemp() { return 1; }`（#159）
...
- [ ] **夹具里的多个计数要互不相同**。审计报告的 summary 有五个计数，若夹具让它们相等
      （都渲染成 5），「正文把 stale 渲染成 missing」这类**串位**在等值断言下是**隐形的**：
      数字对得上，对不上的是它们的归属。2026-09-24 用 1/2/3/4/5 合成夹具后这类变异立刻见红（#158）。
      同理，断言同一组数字时**顺带把「它在哪一处渲染」也钉住**——只比数字等于比对数字与位置都弱
