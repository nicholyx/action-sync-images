...
**本条的落实状态：已落实到四处**（2026-09-20，[#110](https://github.com/nicholyx/action-sync-images/issues/110)）。耗时之外，同一个判据还清掉了另外三处 dry-run 留下的「做过」的痕迹：不写锁文件、不发通知、不记录目标 digest——后两者是参数在本模式下失去了对象，按 `--audit-lock` 的既有模式处理（**不生效 + 告警**，多个参数合并成一条、一次列全，并说明怎么办）。

**例外一处，判据不同、刻意不改**：`sync.sh` 的「总耗时：N 秒」在 dry-run 下照常显示。它描述的是**进程**跑了多久（进程确实跑了那么久，是事实），而且不进报告。判据是「**这条输出描述的是搬运还是进程**」。

断言写在 CI 的「验证 dry-run 不留痕迹」步骤里。**耗时那条必须配 mock `date`**：不带它的话，改动前的代码也能通过（打印一行命令是微秒级），断言等于没写——这是本仓库「恒真的断言比没有断言更糟」的又一具体形态。
